...
namespace Reinkan::Graphics
{
    ...
    void ReinkanApp::BindPyramidalMap(std::vector<std::string> const& filePaths)
    {
        ...
        for (int i =0; i < filePaths.size(); ++i)
        {
            ...
            stbi_uc* pixels = stbi_load(filePaths[i].c_str(), &texWidth, &texHeight, &texChannels, STBI_grey);
            ...
            appPyramidalImageWraps[i] = CreateImageWrap(texWidth,
                                                        texHeight,
                                                        VK_FORMAT_R8_SRGB,                                             // Image Format
            ...
            appPyramidalImageWraps[i].imageView = CreateImageView(appPyramidalImageWraps[i].image, VK_FORMAT_R8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
            appPyramidalImageWraps[i].sampler = CreateTextureSampler(mipLevels);
